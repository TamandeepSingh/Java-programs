...
public class TestParkedCar {
    ...
    public void constructorLicensePlateNumberValidationNegative() {
        ...
        try {
            ParkedCar car = new ParkedCar("Bob Smith", "Porsche", (ParkedCar.CURRENT_YEAR ),null,60);
            fail("license plate number must not be null");
        }
        catch(IllegalArgumentException ex) {
            assertEquals("car license number cannot be null", ex.getMessage());
            grader.addMark(1);
    ...
    public void setModelYearTestNegative() {
        try {
            ...
            fail(" model year must not be earlier than " +ParkedCar.CAR_MODEL_YEAR_LOWER_LIMIT);
        }
        catch (IllegalArgumentException ex) {
            assertEquals("model year cannot be earlier than " + ParkedCar.CAR_MODEL_YEAR_LOWER_LIMIT, ex.getMessage());
            grader.addMark(1);
        ...
        try {
            ...
            fail(" model year must not be later than " +ParkedCar.CAR_MODEL_YEAR_LOWER_LIMIT);
        }
        catch (IllegalArgumentException ex) {
            assertEquals("model year cannot be later than " + ParkedCar.CURRENT_YEAR, ex.getMessage());
            grader.addMark(1);
